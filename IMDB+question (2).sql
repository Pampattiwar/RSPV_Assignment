...
SELECT 
    COUNT(*) AS tot_rowcount_director_mapping
FROM
    director_mapping;
...
SELECT 
    COUNT(*) AS tot_rowcount_role_mapping
FROM
    role_mapping;
...
SELECT 
    COUNT(*) AS TOTAL_No_of_row_director_mapping
FROM
    director_mapping;

-- Total numner of rows = 3876
SELECT 
    COUNT(*) AS TOTAL_No_of_row_genre
FROM
    genre;
...
SELECT 
    COUNT(*) AS TOTAL_No_of_row_movie
FROM
    movie;
...
SELECT 
    COUNT(*) AS TOTAL_No_of_row_names
FROM
    names;
...
SELECT 
    COUNT(*) AS TOTAL_No_of_row_ratings
FROM
    ratings;
...
SELECT 
    COUNT(*) AS TOTAL_No_of_row_role_mapping
FROM
    role_mapping;
...
DESCRIBE movie ;
-- Title, Year, Date_publish, Duration, Country, Worldwide_gross, languges, production have null values
...
(
           SELECT     g.genre,
                      ...
                      CASE
                                 ...
                                 WHEN worlwide_gross_income LIKE '%INR%' THEN Replace(worlwide_gross_income, 'INR ', '') * 0.012
                                 WHEN worlwide_gross_income LIKE '%$%' THEN Replace(worlwide_gross_income, '$ ', '')
                                 ELSE worlwide_gross_income
                      END AS DECIMAL(10) ) AS worlwide_gross_income,
                      Dense_rank() OVER(partition BY m.year ORDER BY Cast(
                      CASE
                                 WHEN worlwide_gross_income IS NULL THEN 0
                                 WHEN worlwide_gross_income LIKE '%INR%' THEN Replace(worlwide_gross_income, 'INR ', '') * 0.012
                                 WHEN worlwide_gross_income LIKE '%$%' THEN Replace(worlwide_gross_income, '$ ', '')
                                 ELSE worlwide_gross_income
                      END AS DECIMAL(38) ) DESC) AS movie_rank
           FROM       genre                      AS g
           INNER JOIN movie                      AS m
           ON         m.id = g.movie_id
           WHERE      g.genre IN
...
WITH multilingualcounts
     AS (SELECT m.id,
                m.production_company,
                Length(m.country) - Length(Replace(m.country, ',', '')) + 1 AS
                   country_count
         FROM   movie AS m),
     productioncompanymoviecount
     AS (SELECT production_company,
                Count(production_company) AS movie_count
         FROM   movie
         GROUP  BY production_company)
SELECT mc.production_company,
       pcmc.movie_count,
       Dense_rank()
         OVER (
           ORDER BY pcmc.movie_count DESC) AS prod_comp_rank
FROM   multilingualcounts AS mc
       INNER JOIN ratings AS r
               ON mc.id = r.movie_id
       LEFT JOIN productioncompanymoviecount AS pcmc
              ON mc.production_company = pcmc.production_company
